...
?>
					...
					<div style="margin: 35px;padding:10px;">
						<h3>Pilih berkas untuk dikirim!</h3>
					</div>
...
<br><br>
<form id="update_batch" action="<?= $a ?>" method="post">
...
<?php 
	...
	foreach($cekberkas as $cb){
		...
		?>
		<tr>
			<td><input type='checkbox' name='update[]' value='<?= $cb['id_usulan'] ?>' ></td>
			<td><?= $cb['nip'] ?></td>
			<td><?= $cb['nama'] ?></td>
			<td><?= $ju ?></td>
			<td>
...
<div class="row mb-3" id="vnip">
	<label for="nosurat" class="col-sm-2 col-form-label" style="text-align:right;">Nomor Surat</label>
	<div class="col-sm-10">
...
<br><br>
<input id="tombol_kirim" type='submit' class="btn btn-primary" form="update_batch" value='Cetak Surat Pengantar' name='batch_update' disabled>
